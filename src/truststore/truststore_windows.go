...
func InstallPlatform() bool {
	// Load cert
	cert, err := ioutil.ReadFile(filepath.Join(misc.CAROOT, misc.rootName))
	misc.FatalIfErr(err, "failed to read root certificate")
	// Decode PEM
	if certBlock, _ := pem.Decode(cert); certBlock == nil || certBlock.Type != "CERTIFICATE" {
		misc.FatalIfErr(fmt.Errorf("invalid PEM data"), "decode pem")
	} else {
		cert = certBlock.Bytes
	}
	// Open root store
	store, err := OpenWinRootStore()
	misc.FatalIfErr(err, "open root store")
	defer store.close()
	// Add cert
	misc.FatalIfErr(store.addCert(cert), "add cert")
	return true
}

//func uninstallPlatform() bool {
...
func OpenWinRootStore() (WindowsRootStore, error) {
